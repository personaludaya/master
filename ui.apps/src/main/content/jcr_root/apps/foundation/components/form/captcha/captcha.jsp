...
<input type="hidden" name="captchakey" id="cq_captchakey" value=""/>
<div class="form_row">
    ...
    <div class="form_rightcol">
        ...
        <div class="form_captcha_img"><img id="cq_captchaimg" src="<%= xssAPI.getValidHref(resource.getPath()) %>.captcha.png?id=123" alt="cq_captchaimg"></div>
        <div class="form_captcha_refresh">
            <%--  <input type="button" onclick="captchaRefresh()" value="<%=i18n.get("refresh")%>" class="<%= FormsHelper.getCss(properties, "form_button") %>"> --%>
            <button class="btn btn-form active" onclick="captchaRefresh(); return false;">            
              <svg class="brands-icon icon-white icon-center">
                <use href="<%=imgPath %>/icons/symbol-defs.svg#icon-refresh" xlink:href="<%=imgPath %>/icons/symbol-defs.svg#icon-refresh"></use>
              </svg>
            </button>            
        </div>
    </div>
