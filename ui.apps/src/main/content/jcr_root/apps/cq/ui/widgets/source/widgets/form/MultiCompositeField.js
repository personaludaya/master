...
[{
            ...
            if (this.fieldWidth > 0) {
                for (var i = 0; i < this.items.length; i++) {
                    ...
                    try {
                        ...
                        //console.log(this.items.get(i));
                        
                        var thisItem = this.items.get(i);
                        //console.log(thisItem);
                        var fields = thisItem.fields.items;
                    	for (var j=0;j<fields.length;j++) {
                    		var obj = fields[j];
                    		
                    		obj.setWidth(this.fieldWidth - 111);
                    	}
                        //item.fieldConfigs.setWidth(this.fieldWidth);
    ...
    processRecord: function(record, path) {
        ...
        if (this.fireEvent('beforeloadcontent', this, record, path) !== false) {
            ...
            if (this.items.length == 1 && this.initialItems > 0) {
                for (var i = 0; i < this.initialItems; i++) {
                    this.addItem();
                }
            }
